...
router.post('/user/:productid/increase', parser, isLoggedIn, async(req, res) => {
    ...
    req.user = await User.findOneAndUpdate({ _id: req.user._id, 'cart.product': productId }, { $inc: { 'cart.$.quantity': 1, totalCartProducts: 1 } }, { new: true }).populate({
        path: 'cart',
        populate: {
            path: 'product'
        }
...
router.post('/user/:productid/decrease', parser, isLoggedIn, async(req, res) => {
    ...
    if (packs[0].quantity === 0) {
        req.user = await User.findByIdAndUpdate(req.user._id, { $pull: { cart: { product: productId } } }, { new: true }).populate({
            path: 'cart',
            populate: {
                path: 'product'
            }
